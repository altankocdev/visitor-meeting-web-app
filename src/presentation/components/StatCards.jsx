...
export function StatCards() { return <div className="stats">{cards.map(({label,value,hint,Icon,tone}) => <article className="stat-card" key={label}><span className={`stat-icon ${tone}`}><Icon /></span><div><small>{label}</small><strong>{value}</strong><p>{hint}</p></div></article>)}</div>; }
