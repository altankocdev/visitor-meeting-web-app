...
export function BookingDialog({ open, onClose, rooms, onCreate }) {
  ...
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" PaperProps={{sx:{borderRadius:4}}}><form onSubmit={handleSubmit(submit)}><DialogTitle sx={{fontWeight:800,fontSize:24}}>Yeni rezervasyon</DialogTitle><DialogContent className={styles.form}>
    ...
    <div className={styles.grid}><Controller name="startTime" control={control} render={({field}) => <TextField {...field} label="Başlangıç" type="time" InputLabelProps={{shrink:true}} fullWidth />} /><Controller name="endTime" control={control} render={({field}) => <TextField {...field} label="Bitiş" type="time" InputLabelProps={{shrink:true}} fullWidth />} /></div>
