...
export function BookingCalendar({ reservations }) {
  ...
  return <section className="panel calendar-panel">
    <div className="panel-head"><div><h3>Toplantı takvimi</h3><p>Ekibinizin haftalık rezervasyon planı</p></div><div className="calendar-tools"><button onClick={() => move("prev")} aria-label="Önceki hafta"><ArrowBackIosNewRounded /></button><button className="today" onClick={() => move("today")}>Bugün</button><button onClick={() => move("next")} aria-label="Sonraki hafta"><ArrowForwardIosRounded /></button><button className="filter"><TuneRounded />Filtrele</button></div></div>
